...
## Режим анонса (сайт закрыт)

Пока идёт пересборка под редизайн, сайт можно закрыть целиком: вместо любой страницы
отдаётся тизер новой версии с контактами (`/soon`, вёрстка — `src/views/SoonPage`).

**Переключатель** — `IS_SITE_CLOSED` в `src/shared/config/soon.ts`:

```ts
export const IS_SITE_CLOSED: boolean = true; // false — сайт открыт
```

Флаг build-time, а не переменная окружения: middleware бандлится в edge-рантайм, где
`process.env` инлайнится на сборке. Править `.env.production` на сервере бесполезно —
включение и выключение только через коммит и деплой.

Что происходит при `true`:

- middleware переписывает любой маршрут на `/soon`, исходный URL остаётся в адресной строке;
- ответ — `503` + `Retry-After: 3600` + `Cache-Control: no-store`. Поисковики понимают это
  как «временно», страницы остаются в индексе (в отличие от `404` или `noindex`);
- локаль берётся из префикса пути (`/en/...` → EN) и уезжает в тизер заголовком
  `x-soon-locale` — сегмента `[locale]` у страницы нет;
- `/api/*`, `robots.txt`, `sitemap.xml`, `feed.xml` и статика продолжают работать:
  telegram-вебхук не теряет посты, а поисковики видят валидный `robots.txt`.

Чтобы открыть сайт: `IS_SITE_CLOSED = false`, затем удалить `src/views/SoonPage`,
`src/app/soon`, `src/shared/config/soon.ts` и ветку с флагом из `src/middleware.ts`.

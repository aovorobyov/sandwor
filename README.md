...
Подтверждение сделано **через мета-тег**: токен лежит в
`metadata.verification.google` (`src/app/layout.tsx`), Next.js рендерит из него
`‹meta name="google-site-verification"›` в `‹head›` всех страниц. Работает без
доступа к DNS — Google читает тег прямо со страницы.

Чтобы подтвердить (или сменить домен):
...
1. В [Search Console](https://search.google.com/search-console) добавь ресурс
   ...
   **«HTML-тег»**.
2. Скопируй токен из `content="…"`, замени им значение в
   `metadata.verification.google`, задеплой.
3. В GSC нажми «Подтвердить». Мета-тег общий для обоих доменов, поэтому одним
   токеном подтверждаются и `sandwor.online`, и `sandwor.ru`.
